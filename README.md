...
- **Sistema de alias y fuente única de verdad:** Los filtros unificados se manejan desde `useFilters` + el array `FILTERS` (`lib/filters/types.ts`). La terminal, los dropdowns, el buscador y las herramientas de IA convergen en el mismo hook. Además, los tres comparten la misma capa de consultas centralizadas en `lib/pokemon/` (con caché, deduplicación y precarga), de modo que un comando de terminal, un filtro del dropdown o una tool call del agente IA ejecutan exactamente la misma query GraphQL contra PokeAPI.
